    ...
    <Container class="tw-flex tw-flex-col tw-items-center">
      <div class="register-main tw-flex tw-flex-col tw-gap-6">
        <div class="register__image">
          <img :src="Logo" alt="">
          <h1>
            {{ showOTPInput ? 'Xác thực OTP' : 'Đăng ký tài khoản' }}
          </h1>
        </div>

        <!-- OTP Input Section -->
        <div v-if="showOTPInput" class="tw-w-full">
            <OTPInput 
            ...
            <div class="tw-flex tw-justify-center tw-mt-4">
                <button @click="showOTPInput = false" class="tw-text-sm tw-text-gray-500 hover:tw-text-red-500 tw-transition-colors">
                    Quay lại đăng ký
                </button>
            </div>
        </div>
        ...
        <div v-else class="tw-w-full">
            <p v-if="registerError" class="tw-text-red tw-text-sm tw-mt-0.5 tw-block tw-self-center tw-text-center">
            {{ (registerError as IError)?.message }}
            </p>
            <p v-if="registerData" class="tw-text-success tw-text-sm tw-mt-0.5 tw-block tw-self-center tw-text-center">
            ...
            <button :disabled="isRegisterLoading"
                class="btn-form__submit tw-text-center hover:tw-opacity-75 tw-transition-all tw-cursor-pointer disabled:tw-opacity-50 disabled:tw-cursor-not-allowed">
                {{ isRegisterLoading ? 'Đang đăng ký...' : 'Đăng ký' }}
            </button>
            </form>
            <div class="line-through tw-flex tw-gap-2 tw-items-center tw-mt-6">
            <div class="tw-flex-1">
            </div>
            ...
            <div class="register-option tw-flex tw-justify-center tw-gap-8 tw-mt-4">
