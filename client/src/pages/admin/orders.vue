<template>
  <div class="tw-p-6 tw-bg-white tw-min-h-screen">
    ...
    <!-- KPI Cards -->
    <div
      class="tw-grid tw-grid-cols-1 md:tw-grid-cols-2 lg:tw-grid-cols-5 tw-gap-4 tw-mb-6"
    >
      <div
        class="tw-bg-white tw-p-4 tw-rounded-xl tw-shadow-md tw-border tw-border-gray-200"
      >
        <div class="tw-flex tw-flex-col tw-items-center tw-gap-3">
          ...
          <div class="tw-text-center">
            ...
            <p class="tw-text-2xl tw-font-bold tw-text-crimson-600">
              {{ stats.todayOrders || 0 }}
            </p>
          </div>
        </div>
      ...
      >
        <div class="tw-flex tw-flex-col tw-items-center tw-gap-3">
          ...
          <div class="tw-text-center">
            ...
            <p class="tw-text-2xl tw-font-bold tw-text-blue-600">
      ...
      >
        <div class="tw-flex tw-flex-col tw-items-center tw-gap-3">
          ...
          <div class="tw-text-center">
            ...
            <p class="tw-text-2xl tw-font-bold tw-text-orange-600">
    ...
    <div class="tw-grid tw-grid-cols-1 lg:tw-grid-cols-2 tw-gap-6 tw-mb-6">
      <!-- Revenue Chart -->
      <div
        class="tw-bg-white tw-p-6 tw-rounded-xl tw-shadow-lg tw-border tw-border-gray-200"
      >
    ...
    <div class="tw-flex tw-justify-between tw-items-center tw-mb-4">
      <div class="tw-relative tw-w-80">
        ...
        <button
          @click="loadOrders"
          class="tw-absolute tw-left-3 tw-top-1/2 tw-transform -tw-translate-y-1/2 tw-text-gray-500 hover:tw-text-crimson-600 tw-transition-colors"
        >
          <svg
            class="tw-w-5 tw-h-5"
            fill="none"
      ...
      <div class="tw-flex tw-gap-2"></div>
    </div>

    <!-- Filter Section -->
    <div
      class="tw-bg-stone-50 tw-border tw-border-stone-200 tw-rounded-lg tw-p-4 tw-mb-4"
    >
      ...
      <div class="tw-flex tw-flex-wrap tw-gap-3">
        ...
        <div class="tw-relative">
          <button
            @click="toggleDropdown('orderStatus')"
          ...
          >
            ...
            <span
              v-if="selectedOrderStatuses.length > 0"
              class="tw-bg-crimson-100 tw-text-crimson-600 tw-px-2 tw-py-0.5 tw-rounded-full tw-text-xs"
              >{{ selectedOrderStatuses.length }}</span
            >
            <svg
              class="tw-w-4 tw-h-4 tw-ml-auto"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
          ...
          >
            <div class="tw-space-y-2 tw-mb-3">
              <label
                ...
                :key="status.value"
                class="tw-flex tw-items-center tw-gap-2 tw-cursor-pointer hover:tw-bg-stone-50 tw-p-2 tw-rounded"
              >
                <input
                  type="checkbox"
                  :checked="tempOrderStatuses.includes(status.value)"
                  @change="toggleOrderStatus(status.value)"
                  class="tw-w-4 tw-h-4 tw-text-crimson-600 focus:tw-ring-2 focus:tw-ring-crimson-500"
                />
            ...
            <div class="tw-flex tw-gap-2 tw-pt-2 tw-border-t">
              <button
                @click="applyFilters"
                class="tw-flex-1 tw-px-3 tw-py-1.5 tw-bg-crimson-600 tw-text-white tw-rounded hover:tw-bg-crimson-700 tw-text-sm tw-font-medium"
              >
                Áp dụng
              </button>
              <button
                @click="closeDropdown"
                class="tw-flex-1 tw-px-3 tw-py-1.5 tw-bg-stone-200 tw-rounded hover:tw-bg-stone-300 tw-text-sm tw-font-medium"
              >
                Hủy
              </button>
            </div>
        ...
        <div class="tw-relative">
          <button
            @click="toggleDropdown('paymentStatus')"
            class="tw-px-4 tw-py-2 tw-bg-white tw-border tw-border-stone-300 tw-rounded-lg hover:tw-bg-stone-50 tw-transition-colors tw-flex tw-items-center tw-gap-2 tw-min-w-[180px]"
          >
            <span class="tw-font-medium">Thanh toán</span>
            <span
              v-if="selectedPaymentStatuses.length > 0"
              class="tw-bg-crimson-100 tw-text-crimson-600 tw-px-2 tw-py-0.5 tw-rounded-full tw-text-xs"
              >{{ selectedPaymentStatuses.length }}</span
            >
            <svg
              class="tw-w-4 tw-h-4 tw-ml-auto"
              fill="none"
          ...
          <div
            v-if="openDropdown === 'paymentStatus'"
            class="tw-absolute tw-mt-2 tw-bg-white tw-shadow-lg tw-border tw-border-stone-200 tw-rounded-lg tw-p-4 tw-z-10 tw-w-64"
          >
            <div class="tw-space-y-2 tw-mb-3">
              <label
              ...
              >
                <input
                  type="checkbox"
                  :checked="tempPaymentStatuses.includes(status.value)"
                  @change="togglePaymentStatus(status.value)"
                  class="tw-w-4 tw-h-4 tw-text-crimson-600 focus:tw-ring-2 focus:tw-ring-crimson-500"
                />
                <span :class="status.color">{{ status.label }}</span>
              </label>
        ...
        <div class="tw-flex tw-gap-2 tw-items-center">
          <input
            ...
            v-model="dateFrom"
            @change="loadOrders"
            class="tw-border tw-border-stone-300 tw-p-2 tw-rounded-lg focus:tw-outline-none focus:tw-ring-2 focus:tw-ring-crimson-500"
          />
      ...
      <table class="tw-min-w-full tw-border">
        ...
        <tbody>
          <tr
            ...
            :key="order._id"
            class="tw-border-b hover:tw-bg-stone-50 tw-transition-colors"
          >
            ...
            <td class="tw-px-4 tw-py-3 tw-font-mono tw-text-sm">
              {{ order.orderNumber }}
            </td>
            <td class="tw-px-4 tw-py-3">
            ...
            <td class="tw-px-4 tw-py-3 tw-font-semibold tw-text-crimson-600">
            ...
            <td class="tw-px-4 tw-py-3">
              <div class="tw-flex tw-gap-2">
                <button
                  ...
                  class="tw-px-3 tw-py-1 tw-bg-blue-600 tw-text-white tw-rounded hover:tw-bg-blue-700 tw-text-sm"
                >
                  Chi tiết
                </button>
                <button
                  @click="openChangeStatusModal(order)"
                  class="tw-px-3 tw-py-1 tw-bg-orange-600 tw-text-white tw-rounded hover:tw-bg-orange-700 tw-text-sm"
                >
                  Đổi trạng thái
                </button>
              </div>
    ...
    <div class="tw-flex tw-justify-between tw-items-center tw-mt-4">
      <div>
        ...
        <select
          v-model="perPage"
          @change="loadOrders"
          class="tw-border tw-border-stone-300 tw-p-1 tw-rounded-lg tw-ml-2 focus:tw-outline-none focus:tw-ring-2 focus:tw-ring-crimson-500 focus:tw-border-transparent"
        >
      ...
      <div class="tw-flex tw-gap-2 tw-items-center">
        <button
          @click="prevPage"
          :disabled="page === 1"
          class="tw-px-4 tw-py-2 tw-bg-crimson-600 tw-text-white tw-rounded-lg hover:tw-bg-crimson-700 disabled:tw-bg-gray-300 disabled:tw-cursor-not-allowed"
        >
          Trước
        </button>
    ...
    <div
      ...
      class="tw-fixed tw-inset-0 tw-bg-black tw-bg-opacity-50 tw-flex tw-items-center tw-justify-center tw-z-50 tw-overflow-y-auto"
    >
      <div
        class="tw-bg-white tw-rounded-lg tw-w-[900px] tw-max-h-[90vh] tw-overflow-y-auto tw-m-4"
      >
        <div
          class="tw-sticky tw-top-0 tw-bg-white tw-border-b tw-p-6 tw-flex tw-justify-between tw-items-center"
        >
          <h2 class="tw-text-xl tw-font-bold tw-text-crimson-600">
        ...
        <div class="tw-p-6">
          <!-- Customer Info -->
          <div class="tw-mb-6">
            <h3 class="tw-text-lg tw-font-semibold tw-mb-3 tw-text-gray-800">
              1️⃣ Thông tin khách hàng
            </h3>
            <div
              class="tw-bg-stone-50 tw-p-4 tw-rounded-lg tw-grid tw-grid-cols-2 tw-gap-4"
            >
              <div>
            ...
            <table class="tw-w-full tw-border">
              ...
              <tbody>
                ...
                >
                  ...
                  <td class="tw-px-4 tw-py-2">
                    ...
                    <p
                      v-if="item.variant?.color || item.variant?.memory"
                  ...
                  <td class="tw-px-4 tw-py-2 tw-text-right tw-font-semibold">
              ...
              <tfoot class="tw-bg-gray-50">
                <tr>
                  <td
                    colspan="4"
                    class="tw-px-4 tw-py-2 tw-text-right tw-font-medium"
                  >
                    Tổng tiền hàng:
                  </td>
                  <td class="tw-px-4 tw-py-2 tw-text-right tw-font-semibold">
                    {{ formatCurrency(selectedOrder?.itemsPrice || 0) }}
                  </td>
                </tr>
                <tr>
                  <td
                  ...
                  <td class="tw-px-4 tw-py-2 tw-text-right">
                    {{ formatCurrency(selectedOrder?.shippingPrice || 0) }}
                  </td>
                </tr>
                <tr v-if="selectedOrder?.discountAmount > 0">
                  <td
                    ...
                    class="tw-px-4 tw-py-2 tw-text-right tw-font-medium"
                  >
                    Giảm giá ({{ selectedOrder?.couponCode }}):
                  </td>
                  <td class="tw-px-4 tw-py-2 tw-text-right tw-text-crimson-600">
                ...
                <tr class="tw-bg-crimson-50">
                  <td
                    colspan="4"
                    class="tw-px-4 tw-py-2 tw-text-right tw-font-bold tw-text-lg"
                  >
                    Tổng cộng:
                  </td>
                  <td
                    class="tw-px-4 tw-py-2 tw-text-right tw-font-bold tw-text-lg tw-text-crimson-600"
                  >
          ...
          <div class="tw-mb-6">
            ...
            >
              ...
              <div
                v-if="selectedOrder?.paymentResult?.transactionId"
                class="tw-col-span-2"
              >
                <p class="tw-text-sm tw-text-gray-600">Mã giao dịch:</p>
                <p class="tw-font-mono tw-text-sm">
          ...
          <div class="tw-mb-6">
            <h3 class="tw-text-lg tw-font-semibold tw-mb-3 tw-text-gray-800">
              4️⃣ Nhật ký trạng thái
            </h3>
            <div class="tw-space-y-3">
              <div
                v-for="(history, index) in selectedOrder?.statusHistory"
                :key="index"
                class="tw-flex tw-gap-4 tw-items-start"
              >
                <div class="tw-flex tw-flex-col tw-items-center">
                  ...
                  <div
                    ...
                    class="tw-w-0.5 tw-h-12 tw-bg-gray-300"
                  ></div>
                </div>
                <div class="tw-flex-1 tw-pb-4">
                  <div class="tw-flex tw-justify-between tw-items-start">
                    <div>
                      <span
                        :class="getOrderStatusClass(history.status)"
                        class="tw-px-2 tw-py-1 tw-rounded-full tw-text-xs tw-font-medium"
                      >
                      ...
                      <p
                        v-if="history.note"
                        class="tw-text-sm tw-text-gray-600 tw-mt-1"
                      >
                        {{ translateStatusInNote(history.note) }}
                      </p>
                    </div>
          ...
          <!-- Notes -->
          <div
            v-if="selectedOrder?.notes || selectedOrder?.cancelReason"
            class="tw-mb-4"
          >
            <h3 class="tw-text-lg tw-font-semibold tw-mb-3 tw-text-gray-800">
              Ghi chú
            ...
            <div class="tw-bg-amber-50 tw-p-4 tw-rounded-lg">
              <p v-if="selectedOrder?.notes" class="tw-text-sm">
                {{ selectedOrder.notes }}
              </p>
              <p
                v-if="selectedOrder?.cancelReason"
                class="tw-text-sm tw-text-crimson-600 tw-font-medium"
              >
    ...
    >
      <div class="tw-bg-white tw-p-6 tw-rounded-lg tw-w-[500px]">
        <h2 class="tw-text-xl tw-font-bold tw-text-crimson-600 tw-mb-4">
        ...
        <div class="tw-mb-4">
          <label
            ...
            >Mã đơn hàng</label
          >
          <p class="tw-font-mono tw-text-sm tw-bg-gray-100 tw-p-2 tw-rounded">
        ...
        <div class="tw-mb-4">
          ...
          <span
            ...
            class="tw-px-3 tw-py-1 tw-rounded-full tw-text-sm tw-font-medium"
          >
    ...
    >
      <div class="tw-bg-white tw-p-6 tw-rounded-lg tw-w-[500px]">
        ...
        <div
          class="tw-mb-4 tw-bg-amber-50 tw-border tw-border-amber-200 tw-p-4 tw-rounded-lg"
        >
          <div class="tw-flex tw-items-start tw-gap-3">
            <svg
              class="tw-w-5 tw-h-5 tw-text-amber-600 tw-flex-shrink-0 tw-mt-0.5"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
            ...
            <div>
              <p class="tw-text-sm tw-font-medium tw-text-amber-800 tw-mb-1">
                Cảnh báo
              </p>
    ...
    <div
      ...
      class="tw-fixed tw-inset-0 tw-bg-black tw-bg-opacity-50 tw-flex tw-items-center tw-justify-center tw-z-[60]"
    >
      <div class="tw-bg-white tw-p-6 tw-rounded-lg tw-w-96 tw-shadow-xl">
        ...
        <button
          ...
          class="tw-w-full tw-px-4 tw-py-2 tw-bg-crimson-600 tw-text-white tw-rounded-lg hover:tw-bg-crimson-700 tw-font-medium"
        >
          Đóng
        </button>
      </div>
...
const loadOrders = async () => {
  try {
    ...
    showNotification(
      error.response?.data?.message || "Lỗi tải danh sách đơn hàng",
      "error"
    );
  }
};
...
function toggleDropdown(type) {
  ...
  if (type === "orderStatus")
    tempOrderStatuses.value = [...selectedOrderStatuses.value];
  if (type === "paymentStatus")
    tempPaymentStatuses.value = [...selectedPaymentStatuses.value];
}
...
async function confirmChangeStatus() {
  ...
  try {
    const response = await axios.patch(
      `${API_URL}/${selectedOrder.value._id}/status`,
      {
...
async function confirmCancelOrder() {
  ...
  try {
    const response = await axios.post(
      `${API_URL}/${selectedOrder.value._id}/cancel`,
      {
    ...
    if (response.success) {
      ...
      if (response.refundInfo) {
        const refundMsg = `Hoàn tiền: ${formatCurrency(
          response.refundInfo.amount
        )} - ${response.refundInfo.method} - ${formatDateTime(
          response.refundInfo.refundedAt
        )}`;
        setTimeout(() => {
