...
class EmailService {
     ...
     */
    async sendOrderStatusEmail(order, status, customerEmail, customerName, refundAmount = 0) {
        try {
            ...
            const { subject, html } = this.getEmailContent(order, status, customerName, refundAmount);
     ...
     */
    getEmailContent(order, status, customerName, refundAmount = 0) {
        ...
        const statusConfig = {
            ...
            cancelled: {
                ...
                message: refundAmount > 0
                    ? `Đơn hàng của bạn đã bị hủy. Chúng tôi đã hoàn lại <strong>${formatCurrency(refundAmount)}</strong> vào tài khoản thanh toán của bạn.`
                    : (order.cancelReason ? `Đơn hàng của bạn đã bị hủy. Lý do: ${order.cancelReason}` : 'Đơn hàng của bạn đã bị hủy.'),
                icon: '❌'
            },
            returned: {
        ...
        const html = generateOrderEmailTemplate({
            ...
            trackingUrl,
            status,
            refundAmount // Pass refundAmount to template
        });
